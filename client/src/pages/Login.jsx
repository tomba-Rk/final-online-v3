...
const Login = () => {
  ...
  const generateReferralLink = async (userId) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/api/create-referral`, { userId: userId,origin: window.location.origin });
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        if (referralId) {
          ...
          const verifyResponse = await axios.post(`${process.env.REACT_APP_API_URL}/api/verifyReferral`, {
